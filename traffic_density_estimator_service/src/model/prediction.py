...
class PredictionHandler:
    ...
    def __init__(self, weights=None):
        super(PredictionHandler, self).__init__()

        load_custom_weights = False if weights is None else True
        self.model = DensityEstimator(load_weights=load_custom_weights)
        if weights is not None:
            logging.info(f"Resuming from a checkpoint")
            checkpoint = torch.load(weights, map_location="cpu")
            self.model.load_state_dict(checkpoint['model'])

    def predict(self, x, bmask=None):
        with torch.no_grad():
            dmap = self.model(x, bmask=bmask)
